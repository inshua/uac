(function($) {
    ...
    var methods = {
        ...
        onStart: function(event) {
            ...
            if ($element) {
            	// console.log('trigger long click');            	
            	$element.mayTriggerLongClicks().on('longClick', function() {
	                $sourceElement = $element;
	                if($sourceElement.data('dragging') == 1) return;
	                $sourceElement.data('dragging', 1);
	                ...
	                if (options.makeClone) {
	                    ...
	                    $activeElement.appendTo(options.parentElement || $element.parent());
	                    // $activeElement.appendTo($('body'));
        ...
        onMove: function(event) {
            ...
            //console.log('hover ' + destElement.className);
            ...
            var zoom = $activeElement.css('zoom') * 1;
            $activeElement.css({ left: posX / zoom, top: posY / zoom });
            ...
            if (destElement) {
                if ($destElement==null || $destElement.get(0)!= destElement) {
                    ...
                    if (options.canDrop($possibleDestElement)) {
                        ...
                        if (options.canDropClass) {
                        	//console.log('add class ' + options.canDropClass);  
        ...
        onEnd: function(event) {
            ...
            $sourceElement.data('dragging', null);
